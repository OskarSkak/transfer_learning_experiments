...
class TransferLearningModel():
    ...
    def add_fully_connected_layers(self, dense_activation='relu', pred_activation='softmax', dropout_rate = 0):
        ...
        self.model = models.Sequential([self.base_model])
    ...
    def m2texbase(self, name):
        ...
        print('\n\n\n')
        table.to_csv(f'./models_csv/DERM{name}.csv', encoding='utf-8', index=False)
...
def main():
    ...
    model.load_data(150, 250, percentage_split)
    ...
    #     gpu_train.time = count/10 # (polled every five seconds)
    #     gpu_train.usage = gpu
    #     gpu_train.load = load
    ...
    #     gpu_test.time = count/10 # (polled every five seconds)
    #     gpu_test.usage = gpu
    #     gpu_test.load = load
